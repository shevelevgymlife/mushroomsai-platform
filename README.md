...
### Ops-уведомления (6 блоков) в `@MushroomsAi_system_bot`

В проекте включены блоки ops-уведомлений:
1. infra/deploy (старт/финиш деплоя)
2. billing (дата ближайшего платежа, приближение к лимиту)
3. payments/orders (новые заказы)
4. operations (feedback, вопросы по товарам, запросы смены тарифа)
5. security (ошибки/аномалии авторизации)
6. daily summary (ежедневная сводка за 24 часа)

Минимальные env для этого:

```bash
OPS_TELEGRAM_TOKEN=...                 # токен системного бота
DEPLOY_NOTIFY_TG_CHAT_ID=...           # chat id, куда слать ops/deploy уведомления
OPS_NOTIFY_DAILY_SUMMARY_HOUR_UTC=9    # час UTC ежедневной сводки
OPS_NOTIFY_BILLING_DUE_AT=2026-04-01   # опционально, дата платежа YYYY-MM-DD
OPS_NOTIFY_BILLING_CURRENT_USD=0       # опционально, текущие расходы
OPS_NOTIFY_BILLING_LIMIT_USD=0         # опционально, лимит
OPS_NOTIFY_BILLING_WARN_PERCENT=90     # warning порог
```
