...
- **Jinja2** + **Tailwind CSS** (собранный `static/css/tailwind-app.css`, без CDN) — шаблоны и UI
- **OpenAI GPT-4o** — AI
...
После изменений классов в `web/templates/**/*.html` или `static/js/**/*.js` пересоберите стили (файл `static/css/tailwind-app.css` коммитится в репозиторий — на Render **Node не нужен**):
...
Windows при блокировке `npm.ps1`: `cmd /c "npm.cmd install && npm.cmd run build:css"`. Конфиг темы: `tailwind.config.js`.
