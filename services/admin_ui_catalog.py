...
_ADMIN_ITEMS_RAW: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "perm": "can_groups",
        "subtitle": "Виджет NeuroFungi AI в Telegram-группах",
        "help": (
            "<p><b>Что это:</b> закреплённое сообщение в Telegram-группе или супергруппе с кнопкой входа в бота (вопрос к AI) "
            "и при необходимости — ссылкой на магазин партнёра. Включение только отсюда: сначала глобальный переключатель, "
            "затем отдельно каждый чат.</p>"
            "<p><b>Рефералы:</b> те же правила, что у кнопки «в соцсеть» у владельца канала: персональная ссылка на бота при активной партнёрке, "
            "поле <code>referral_shop_url</code> у пользователя или регистрация <code>/login?ref=</code>.</p>"
            "<p><b>На что влияет:</b> только чаты, куда добавлен бот и где вы явно включили виджет; по умолчанию всё выключено.</p>"
        ),
    },
    {
        "id": "video_calls",
    ...
    },
    {
        ...
        "perm": "can_ai",
        "subtitle": "Кабинет, Telegram, лента, дневник, переопределения",
        "help": (
            "<p><b>Что это:</b> отдельные инструкции, тон и флаги для каждого типа общения AI; глобально и JSON-переопределение на пользователя.</p>"
            "<p><b>На что влияет:</b> дополнительный системный текст в выбранных сценариях; ЛС/посты/комментарии могут перекрыть блок на /admin/ai.</p>"
        ),
    },
    {
        "id": "ai_posts",
