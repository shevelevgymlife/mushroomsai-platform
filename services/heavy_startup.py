...
async def run_heavy_startup(app: FastAPI) -> None:
    try:
        ...
        new_columns = [
            ...
            "ALTER TABLE community_comments ADD COLUMN IF NOT EXISTS parent_comment_id INTEGER REFERENCES community_comments(id) ON DELETE SET NULL",
        ...
        try:
            import migrate_v32_wellness_analytics as migrate_v32

            for s in migrate_v32.STEPS:
                await database.execute(sa.text(s))
            logger.info("wellness analytics (migrate_v32) OK")
        except Exception as e:
        ...
        try:
            import migrate_v35_wellness_states_schemes_rl as migrate_v35

            for s in migrate_v35.STEPS:
                await database.execute(sa.text(s))
            logger.info("wellness states/schemes/RL (migrate_v35) OK")
        except Exception as e:
            logger.warning("migrate_v35 wellness_states_schemes_rl: %s", e)
        try:
            import migrate_v36_wellness_admin_ai_silent as migrate_v36

            for s in migrate_v36.STEPS:
                await database.execute(sa.text(s))
        ...
        try:
            import migrate_v42_referral_payout as migrate_v42

            for s in migrate_v42.STEPS:
                await database.execute(sa.text(s))
            logger.info("referral payout profile & withdrawal month (migrate_v42) OK")
        except Exception as e:
        ...
        try:
            import migrate_v50_nfi_decimal_withdraw as migrate_v50

            for s in migrate_v50.STEPS:
                await database.execute(sa.text(s))
