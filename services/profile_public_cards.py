"""Настройки публичных свайп-блоков профиля (о себе + соцсети; токены — в блоке «о себе»)."""
...
SLIDE_KEYS: tuple[str, ...] = ("about", "social")


def default_profile_public_cards() -> dict[str, Any]:
    return {
        "show_crypto_in_about": True,
...
def normalize_slide_order(raw: Any) -> list[str]:
    """Порядок about/social; legacy «crypto» из старых данных отбрасывается."""
...
def merge_profile_public_cards(raw: str | None) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        return base
    if "show_crypto_in_about" in data:
        base["show_crypto_in_about"] = bool(data["show_crypto_in_about"])
    elif "show_crypto_slide" in data:
        base["show_crypto_in_about"] = bool(data["show_crypto_slide"])
...
def dumps_profile_public_cards(data: dict[str, Any]) -> str:
    ...
    clean["show_crypto_in_about"] = bool(data.get("show_crypto_in_about"))
...
def profile_public_cards_from_form(
    show_crypto_in_about: bool,
    show_social: bool,
    social_raw: dict[str, str],
    slide_order_csv: str = "",
) -> str:
    d = default_profile_public_cards()
    d["show_crypto_in_about"] = show_crypto_in_about
